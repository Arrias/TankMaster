...
void GameDrawer::draw_game(sf::RenderWindow &window) {
    ...
    for (auto block : game->get_blocks()) {
        auto texture = texture_loader->load_item(WALL, id_to_texture[block->get_id()]);
        draw_object(block.get(), window, texture);
    }

    for (auto tank : game->get_tanks()) {
        auto texture = texture_loader->load_item(TANK, id_to_texture[tank->get_id()]);
    ...
    for (auto bullet : game->get_bullets()) {
        ...
        auto texture = texture_loader->load_item(BULLET, 1);
        draw_object(bullet.get(), window, texture);
    }
}

void GameDrawer::draw_floor(sf::RenderWindow &window) {
    auto floor_texture = texture_loader->load_item(FLOOR, floor_type);
...
GameDrawer::GameDrawer(Game *game, size_t floor_type, Loader<sf::Texture> *texture_loader) : game(game), floor_type(floor_type),
