...
void MainMenu::show() {
    ...
    auto &multiplayer_game_button = buttons[1];
    auto &add_room_button = buttons[2];
    auto &join_room_button = buttons[3];
    auto &exit_button = buttons[4];
    ...
    multiplayer_game_button.setCallback([&window, this]() {
        window.close();
        pars.nav->push_back(shared_ptr<Window>(new UdpMultiplayerGame(Window(pars))));
    });
...
MainMenu::MainMenu(Window base) : Window(base) {
    ...
    buttons = {
            ...
            Button(Block(Vector(title_pos.x, title_pos.y + button_margin * 4),
                         button_size, 0, 0),
                   button_texture, active_button_texture, get_button_with_text("Multiplayer Game")),
            ...
            Button(Block(Vector(title_pos.x, title_pos.y + button_margin * 7),
                         button_size, 0, 0),
